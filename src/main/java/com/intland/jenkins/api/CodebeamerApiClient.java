...
public class CodebeamerApiClient {
	...
	// public void postTestRuns(TestResults tests, AbstractBuild<?, ?> build)
	// throws IOException {
	// String buildIdentifier = this.getBuildIdentifier(build);
	//
	// TrackerItemDto[] testCases =
	// this.getTrackerItems(this.pluginConfiguration.getTestCaseTrackerId());
	...
	// Integer testCaseId =
	// this.findOrCreateTrackerItemInTree(test.getFullName(),
	...
	// }
	//
	// TrackerItemDto parentItem = this.createParentTestRun(tests,
	// buildIdentifier, build,
	// this.pluginConfiguration.getTestConfigurationId(), testSetId,
	// testCasesForCurrentTestRun.values());
	//
	// int uploadCounter = 0;
	// for (TestResultItem test : tests.getTestResultItems()) {
	// Integer testCaseId = testCasesForCurrentTestRun.get(test.getFullName());
	// TrackerItemDto createdRun =
	// this.createTestRun(this.pluginConfiguration.getTestConfigurationId(),
	// testSetId,
	// parentItem, test, testCaseId);
	// this.updateTestRunStatusAndSpentTime(createdRun.getId(), "Completed",
	// (long) (test.getDuration() * 1000));
	//
	// uploadCounter++;
	...
	// }
	//
	// this.updateTestSetTestCases(testSetId,
	// testCasesForCurrentTestRun.values());
	...
	// }
	//
	// private TrackerItemDto createTestRun(Integer testConfigurationId, Integer
	// testSetId, TrackerItemDto parentItem,
	// TestResultItem test, Integer testCaseId) throws IOException {
	// TestRunDto testRunDto = new TestRunDto(test.getName(),
	// parentItem.getId(),
	// this.pluginConfiguration.getTestRunTrackerId(), Arrays.asList(new
	// Integer[] { testCaseId }),
	...
	// private String createParentMarkup(AbstractBuild<?, ?> build) throws
	// IOException {
	// long currentTime = System.currentTimeMillis();
	// BuildDto buildDto = BuildDataCollector.collectBuildData(build,
	// currentTime);
	// ScmDto scmDto = ScmDataCollector.collectScmData(build, this);
	// TestResultDto testResultDto =
	// TestResultCollector.collectTestResultData(build, this.listener);
	//
	// return new
	// WikiMarkupBuilder().initWithTestReportTemplate().withBuildInfo(buildDto)
	// .withTestReportInfo(testResultDto).withScmInfo(scmDto).build();
	// }

	// private TrackerItemDto createParentTestRun(TestResults tests, String
	// buildIdentifier, AbstractBuild<?, ?> build,
	// Integer testConfigurationId, Integer testSetId, Collection<Integer>
	// testCaseIds) throws IOException {
	// String newMarkupContent = this.createParentMarkup(build);
	// TrackerItemDto parentItem;
	// TestRunDto parentRunDto = new TestRunDto(buildIdentifier, null,
	// this.pluginConfiguration.getTestRunTrackerId(),
	// testCaseIds, testConfigurationId, tests.getStatus());
	// parentRunDto.setTestSet(testSetId);
	// parentRunDto.setDescription(tests.getTestSummary().toWikiMarkup() +
	// newMarkupContent);
	...
	// }
	//
	// private Integer findOrCreateTrackerItemInTree(String fullName, Integer
	// trackerId, NodeMapping nodeMapping,
	// Integer folder, Integer limit) throws IOException {
	// fullName = null; // XUnitUtil.limitName(fullName, limit, ".");
	//
	// if ((folder != null) && (nodeMapping.getIdNodeMapping().get(folder) !=
	// null)) {
	...
	// private Integer postTrackerItemWithParent(String path, Integer trackerId,
	...
	// testCaseDto.setType(trackerId.equals(this.pluginConfiguration.getTestCaseTrackerId())
	// ? "Automated" : null);
	...
	// private TrackerItemDto updateTestSetTestCases(Integer testSetId,
	// Collection<Integer> testCases) throws IOException {
	// List<Object[]> testCasesList = new ArrayList<>();
	// for (Integer testCaseId : testCases) {
	// testCasesList.add(new Object[] { new ReferenceDto("/item/" + testCaseId),
	...
	// }
	//
	// private TrackerItemDto updateTestRunStatusAndSpentTime(Integer id, String
